...
struct ListObjects<C, S> { s3: C, bucket: String, stream: S, }
impl<C, S> ListObjects<C, S>
where C: S3 + Clone + Send, S: Stream<Item=Result<ListObjectsV2Output, Error>> + Sized + Send
{
...
where S: Stream<Item=Result<ListObjectsV2Output, Error>> + Sized + Send + Unpin,
      C: Unpin
{
